...
- [ ] WI-CX0058-fix Context Pack Selection Breadth Guard: prevent general path or intent tokens from selecting an unbounded historical record set, using the 123-entry WI-CX0056 and 120-entry WI-CX0057 selections as regression evidence.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| KI-CX-CONTEXT-001 Context-pack selection breadth. | Medium | CODEX | WI-CX0056 selected 123 metadata chunks and WI-CX0057 selected 120 after a general `handoff` token matched historical metadata. | No chunk bodies were stored or carried; only directly relevant selected sources were loaded. | WI-CX0058-fix adds a deterministic breadth guard or more precise matching. | Before generalized automated WI cadence or efficiency claims. | open |
