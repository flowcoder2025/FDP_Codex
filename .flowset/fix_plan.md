...
| --- | --- | --- | --- | --- | --- | --- | --- |
| KI-CX-WORKER-004 / Issue #64 Windows reused parent PID false descendant. | High | CODEX | Repeated host lifecycle tests intermittently classified the no-child normal fixture as residual because the observer did not confirm the current parent identity after PID reuse. | None; an unrelated process could be targeted. | Live parent identity matching, deterministic reused-parent coverage, and five consecutive full lifecycle runs pass. | Before WI-CX0060 PR readiness, merge, dogfood continuation, or generalized managed-worker use. | repaid |
