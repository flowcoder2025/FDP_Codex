...
- [ ] WI-CX0048-test Runtime Snapshot Validator: add repo-visible runtime snapshot validation for parent thread, goal thread, automation, runner thread ids, execution environment, and handoff receiver result evidence before further independent A2/Layer 2 progression. Expected strategy: PSC=P1, WTC=VAL, Risk=R2, ESC=E1+E3+E5+E6.
...
- WI-CX0049-docs A2 Handoff Receiver Contract: define receiver success, duplicate-stop handling, and repo-visible reporting before generalized A2/A3 autonomy expansion.
- WI-CX0050-test Worktree Isolation Verification: prove or block the A2 worktree isolation model before first Layer 2 target-project scaffold confidence claims.
- WI-CX0038-docs Layer 2 Scope Code Accepted Decision: start only after the user chooses the Layer 2 project scope code rule and after WI-CX0048 installs control-plane confidence checks.
