...
Reviewer history is evidence, not authority; exact review generations are recorded in `.flowset/current-wi.md`, `.flowset/state.json`, and `docs/records/validation-wi-cx0060-test.md`. The latest completed review on exact head `3561edff6723f02deb1c5963c23964f9e4e2e778` returned FAIL with one P1: a newly discovered descendant without start-time metadata could enter the observed tree and later receive a targeted signal. The active remediation keeps that PID outside the observed tree, reports it as an unknown candidate, permanently marks observation unverified, and adds a deterministic regression. A fresh clean-head review remains required, so Issue #63 blocks PASS and PR readiness.
