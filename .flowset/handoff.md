...
The official ChatGPT-login managed preflight passed. The first dogfood run reconstructed the target and passed both target validators, then timed out after entering unsupported nested collaboration; managed cleanup verified zero residual processes. The runner now enforces `--disable multi_agent`, verifies 18 supported direct, package-manager, and wrapper re-entry forms through the target exec-policy before reading the prompt, and passes lifecycle tests. The exec-policy is not universal command authorization; Windows process lifetime is separately contained by a kill-on-close Job Object.
...
Reviewer availability was intermittent. Reviewers returned FAIL on heads `b63bbca2552d6fe071812c279143a046683d0ac1`, `7696fbb`, and `c86b9f036e823986d78d825c97408b70dcd444b1`. The latest review found a P1 shell-reentry bypass and P2 Issue #55 drift. Issue #55 was synchronized, and the remediation adds fail-before-prompt exec-policy checks plus an explicit non-universal scope. Repeated host testing also exposed KI-CX-WORKER-004 / Issue #64: Windows parent PID reuse could create a false descendant. Live parent identity matching and deterministic coverage now pass five consecutive lifecycle runs. Reviewer `019f4db0-018f-7db1-b8e4-81c8e1aa92fc` then returned FAIL on head `71576c01ca9a1db1fb59031c01a398bb13e9cba8` with package-script re-entry, premature Issue #64 closure, truncated Issue #63, and Strategy A wording findings. Package managers are now forbidden inside workers, repository validation belongs to the controller, Issue #64 is reopened with required labels and `repaid-on-merge` state, and the complete live Issue #63 KI body is restored before the next review. Reviewer `019f4dc3-db87-7043-b699-b1d1c4145217` then returned FAIL on head `b46b0b745db4f9bde2dcd031e5e11d5d8b54d7cf`: Windows polling could miss a detached child after an immediate parent exit, Issue #64 hard-stop wording contradicted `repaid-on-merge`, and three policy messages suggested forbidden package scripts. The remediation now assigns the real Windows worker to a kill-on-close Job Object before resume, requires a verified drain, covers the fast-parent-exit case, aligns the KI hard stop with post-merge closeout, and returns script execution guidance to the visible controller. The changed head remains unreviewed, so KI-CX-REVIEW-002 / Issue #63 still blocks independent PASS claims.
