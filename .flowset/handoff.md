...
The official ChatGPT-login managed preflight passed. The first dogfood run reconstructed the target and passed both target validators, then timed out after unsupported nested collaboration. Its earlier polling wrapper reported `cleanup.verified: true` and no matched residuals, but later review proved that implementation could miss a fast detached child; this is historical reported evidence, not a complete zero-residual proof. Current process-containment confidence comes from separate deterministic Windows Job Object tests. The runner enforces built-in `--disable multi_agent`, but command re-entry is not runtime-confined.
...
Reviewer history is evidence, not authority. Reviewer 019f56f8-79b2-7b61-bcf2-73bcf6c69cf5 returned PASS on prior head 9c6080043b49ea98f5f29e8b3fe79baf4e10a429, then that evidence became stale. Reviewer 019f576e-9981-7343-a065-555961f62853 returned FAIL on 6675269e74e720f6344d563ec872ed336809f21f. Reviewer 019f5a06-e5e2-7960-92fe-583d9525ba3e returned FAIL on 2cb1976d264d4c621953feb71d38af2dd041e987. Reviewer 019f5a49-75f1-7222-bdfd-f746e9f4cf80 returned FAIL in GitHub review 4682548609 on exact head 376e82dffd9d0bd82d6c8a329ce3a05f64f1f4e4. Reviewer 019f5a78-d8f4-73e0-8744-2f8f070810af returned FAIL in GitHub review 4682944626 on exact head d6c0a68c3520d3ca31fabdba57aa4aeaa370ae6c because runtime compilation created an unmanaged pre-Job compiler and setup-helper lifetime. Reviewer 019f5ae5-5c4c-76c1-97f6-fbd923c0a122 then completed all required commands and returned FAIL in GitHub review 4683739642 on exact head a76f111c0361a0a409ad42c24673ef86249aedb2 with P1 relative command shadowing, P2 prompt-result schema incompleteness, and a same-WI P3 native error-detail disposition. Issues #66 and #67 now track repayment. The current remediation uses canonical absolute CLI resolution with target-root exclusion, complete prompt-boundary watchdog fields, preserved native operation/code/message details, and the existing reproducibly built no-console WindowsApplication runner. PR #65 is already published; query its live head and checks rather than predicting them here.

Reviewer 019f5b27-a680-7d20-8c02-bb5c71eb6bc6 returned FAIL in GitHub review 4684212043 on exact head c2a24c8ce22c47972afd67d30ad8799d4fcc14e1 with two P2 findings: setup resolution failure emitted no `worker.result`, and multiline candidate fields could evade live-head comparison. Issues #68 and #69 track repayment. The current remediation uses one shared null-root setup-failure publisher and continuation-aware candidate parsing with stale full and abbreviated SHA rejection. A changed head requires full canonical validation and another fresh review.
...
WI-CX0057 accepts one visible control task plus `codex exec --ephemeral` workers. The controller owns repository-supplied validation, staged review, branch creation, commit, push, PR, merge, and approvals; workers own SSOT reconstruction and worktree edits only. Workers must not create persistent app tasks or receive `danger-full-access` solely for Git metadata. KI-CX-DOGFOOD-001 is repaid by this boundary, while runner reactivation remains separately gated and unapproved.
...
WI-CX0059 originally implemented `scripts/run-ephemeral-worker.mjs` with stdin-only prompts, JSONL event streaming, finite timeout, interruption handling, process identity tracking, and verified cleanup. Later adversarial review showed the POSIX process-group path could miss a detached fast-exit descendant, so the current runtime supports verified Windows Job Object containment only and fails closed before spawning on other platforms. The Windows deterministic lifecycle and no-model CLI smoke remain the supported proof. KI-CX-WORKER-001 remains repaid for that bounded surface.

Required post-merge validation of WI-CX0059 intermittently classified an older Windows process row with a stale parent pid as a live descendant after the root pid was reused. WI-CX0061 rejects candidates that started before their observed parent or root and adds a deterministic stale-row test. Five consecutive full lifecycle runs passed, repaying KI-CX-WORKER-002 before dogfood or runner reactivation.
...
- WI-CX0057-docs: Ephemeral Worker Controller Boundary Contract. Evidence: `docs/decisions/2026-07-10-ephemeral-worker-controller-boundary.md` and `docs/records/validation-wi-cx0057-docs.md`. Result: a single visible controller owns Git, repository validation, and user interaction while ephemeral workers reconstruct and edit without app task fan-out.
...
Complete the Issues #68 and #69 remediation, run the canonical local validation set, commit and push the changed PR #65 head, then synchronize Issues #63, #64, #66, #67, #68, and #69. Obtain a new clean-context blind adversarial review of that exact published head and run the independent-review plus live control-plane audits. If the exact head passes every required check, merge under the active user approval, then complete branch, Issue, and post-merge audit closeout. Land only as a non-completion checkpoint with truthful `blocked-external` status, keep Issues #55 and #61 open, and do not mark the end-to-end proof complete. After closeout, start WI-CX0064-docs to define the local v0.1 self-hosting completion contract while Issue #62 continues to block further target progression.
...
- KI-CX-WORKER-004 / Issue #64 remains open; the pre-Job compiler gap is locally remediated, but fresh exact-head review, PR validation, merge, and post-merge audit are still required before closure.
- KI-CX-WORKER-005 / Issue #66 remains open; relative and target-root command shadowing plus native error detail are locally remediated, but canonical validation, fresh exact-head review, merge, and post-merge audit remain.
- KI-CX-WORKER-006 / Issue #67 remains open; prompt-boundary schema completeness is locally remediated, but canonical validation, fresh exact-head review, merge, and post-merge audit remain.
- KI-CX-WORKER-007 / Issue #68 remains open; setup-failure terminal-result completeness is locally remediated, but canonical validation, fresh exact-head review, merge, and post-merge audit remain.
- KI-CX-CONTROL-002 / Issue #69 remains open; multiline candidate-field parsing is locally remediated, but canonical validation, fresh exact-head review, merge, and post-merge audit remain.
...
- KI-CX-REVIEW-002 blocks PR readiness and merge because exact head c2a24c8ce22c47972afd67d30ad8799d4fcc14e1 failed in GitHub review 4684212043 with two P2 findings and every remediation commit invalidates older evidence; a fresh exact-head PASS and audit are required.
