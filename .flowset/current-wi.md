...
- Validator stance: require built-in fan-out disabling plus supported exec-policy re-entry checks, repository CI, a completed live dogfood result, zero residual processes, and a separate current-head independent review before merge.
...
- The runner now adds `--disable multi_agent` and requires `.codex/rules/fdp-managed-worker.rules` before reading the prompt. The real CLI preflight forbids 12 supported direct and wrapper re-entry cases, while `npm.cmd run worker:test` passes the policy contract and process lifecycle cases.
