...
On Windows, the supervisor creates the real worker suspended, assigns it to a kill-on-close Job Object, and resumes it only after assignment. Normal completion requires the Job Object to report zero active processes after termination, covering descendants whose parent exits before a metadata poll can observe them. Other platforms fail closed before spawning because equivalent containment is not implemented. If Windows process creation succeeds but Job Object assignment fails, the still-suspended unassigned process is terminated and waited to exit before the failure is returned. The supervisor also records pids, parent pids, executable names, and start times as evidence; identity checks prevent a reused pid from being treated as the original worker.
