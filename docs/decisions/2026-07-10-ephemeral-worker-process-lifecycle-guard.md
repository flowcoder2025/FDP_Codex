...
The native runtime is a prebuilt `WindowsApplication` executable rather than a PowerShell script that compiles C# with `Add-Type`. Its tracked manifest binds the executable hash to the auditable C# source hash, and the visible Node controller checks both before spawn. An explicit build command is the only compiler boundary; managed invocation never starts `csc.exe`. No-console creation applies to the wrapper and worker so neither setup phase adds an unmanaged console host. The pre-run stall regression observes zero temporally valid direct wrapper children before `Run` after excluding older stale-parent rows by creation time, then proves timeout closes the exact wrapper within the finite bound, starts no worker, and classifies the wrapper exactly once. The builder normalizes the source path, uses Roslyn deterministic compilation, records compiler and framework-reference hashes in manifest schema 2, and provides a non-mutating exact-rebuild verification command.
...
After the real worker root exits, the native wrapper gives Job accounting a bounded natural convergence window. If another Job process remains active after that window, including a descendant not yet observed by metadata polling, the wrapper records that active count, terminates and drains the Job, and returns a lifecycle failure instead of the root exit code. Transient Windows process-table acquisition retries inside one absolute observation deadline; persistent uncertainty remains fail closed.
